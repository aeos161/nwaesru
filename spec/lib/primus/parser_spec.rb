RSpec.describe Primus::Parser do
  describe "#parse" do
    ...
    context "the previous page ended with a partial word" do
      it "completes that word" do
        first_word = Primus::Word.new(tokens: [
          Primus::Token::Character.new(lexeme: "ᛇ", location: double),
        ])
        document = Primus::Document.new(text: [first_word])
        tokens = [
          Primus::Token::Character.new(lexeme: "ᚣ", location: double),
          Primus::Token::Character.new(lexeme: "ᚹ", location: double),
          Primus::Token::Character.new(lexeme: "ᚫ", location: double),
          Primus::Token::Character.new(lexeme: "ᚷ", location: double),
          Primus::Token::Character.new(lexeme: "ᛄ", location: double),
          Primus::Token::WordDelimiter.new(location: double)
        ]
        parser = Primus::Parser.new(tokens: tokens, document: document,
